...
def replace_frac(match) -> str:
    ...
    sign_str = "负" if sign else ""
    return f"{sign_str}{num2str(denominator)}分之{num2str(nominator)}"
...
def replace_percentage(match) -> str:
    ...
    sign_str = "负" if sign else ""
    return f"{sign_str}百分之{num2str(percent)}"
...
def replace_negative_num(match) -> str:
    ...
    sign_str = "负" if sign else ""
    return f"{sign_str}{num2str(number)}"
...
def replace_power(match) -> str:
    power_num = "".join(power_map[m] for m in match.group(0))
    return "的" + power_num + "次方"
...
def replace_positive_quantifier(match) -> str:
    ...
    if match_2 == "+":
        match_2 = "多"
    match_2 = match_2 if match_2 else ""
    quantifiers = match.group(3)
    number_str = num2str(number)
    number_str = "两" if number_str == "二" else number_str
    return f"{number_str}{match_2}{quantifiers}"
...
def replace_range(match) -> str:
    first = RE_NUMBER.sub(replace_number, match.group(1))
    second = RE_NUMBER.sub(replace_number, match.group(6))
    return f"{first}到{second}"
...
def replace_to_range(match) -> str:
    return match.group(0).replace("~", "至")

RE_VERSION_NUM = re.compile(r"((\d+)(\.\d+)(\.\d+)?(\.\d+)+)")
def replace_vrsion_num(match) -> str:
    return "".join(num2str(c) if c != "." else "点" for c in match.group(1))
...
def verbalize_digit(value_string: str, alt_one=False) -> str:
    result = "".join(DIGITS[digit] for digit in value_string)
    if alt_one:
...
def num2str(value_string: str) -> str:
    ...
    if len(integer_decimal) == 1:
        integer, decimal = integer_decimal[0], ""
