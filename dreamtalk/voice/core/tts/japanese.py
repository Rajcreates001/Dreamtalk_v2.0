    ...
    if os.name == "nt":
        ...
        if not (re.match(r"^[A-Za-z0-9_/\\:.\-]*$", OPEN_JTALK_DICT_DIR)):
            if OPEN_JTALK_DICT_DIR[:len(python_dir)].upper() == python_dir.upper():
                ...
                import shutil
                os.makedirs("TEMP/ja", exist_ok=True)
        ...
        if not (re.match(r"^[A-Za-z0-9_/\\:.\-]*$", current_file_path)):
            if current_file_path[:len(python_dir)].upper() == python_dir.upper():
                ...
                import shutil
                os.makedirs("TEMP/ja", exist_ok=True)
                os.makedirs(os.path.join("TEMP", "ja", "ja_userdic"), exist_ok=True)
                src_ud = os.path.join(current_file_path, "ja_userdic", "userdict.csv")
                if os.path.exists(src_ud):
                    shutil.copyfile(src_ud, os.path.join("TEMP", "ja", "ja_userdic", "userdict.csv"))
...
_real_sokuon = [(re.compile("%s" % x[0]), x[1]) for x in [
    (r"Q([↑↓]*[kg])", r"k#\1"), (r"Q([↑↓]*[tdjʧ])", r"t#\1"),
    (r"Q([↑↓]*[sʃ])", r"s\1"), (r"Q([↑↓]*[pb])", r"p#\1"),
]]
_real_hatsuon = [(re.compile("%s" % x[0]), x[1]) for x in [
    (r"N([↑↓]*[pbm])", r"m\1"), (r"N([↑↓]*[ʧʥj])", r"n^\1"),
    (r"N([↑↓]*[tdn])", r"n\1"), (r"N([↑↓]*[kg])", r"ŋ\1"),
]]

def post_replace_ph(ph):
    rep_map = {"：": ",", "；": ",", "，": ",", "。": ".", "！": "!", "？": "?", "\n": ".", "·": ",", "、": ",", "...": "…"}
    return rep_map[ph] if ph in rep_map else ph
