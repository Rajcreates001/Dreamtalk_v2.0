...
from .model import KModel, _find_weights_dir
...
class KPipeline:
    ...
    def __init__(
        ...
        Args:
            ...
            weights_dir: Local Kokoro weights dir (offline-first). Falls back to
                         <project>/weights/kokoro, then HuggingFace download.
        """
        if repo_id is None:
        ...
        if isinstance(model, KModel):
            ...
            if device is None:
                if torch.cuda.is_available():
                    ...
                    device = 'cpu'
            try:
                self.model = KModel(repo_id=repo_id, weights_dir=self.weights_dir).to(device).eval()
    ...
    def load_single_voice(self, voice: str):
        ...
        if voice.endswith('.pt'):
            ...
            # Offline-first: prefer local weights/kokoro/voices/<voice>.pt
            local_voice = (
                os.path.join(self.weights_dir, 'voices', f'{voice}.pt')
                if self.weights_dir else None
            )
            if local_voice and os.path.exists(local_voice):
                logger.debug(f"Using local Kokoro voice: {local_voice}")
                f = local_voice
            else:
