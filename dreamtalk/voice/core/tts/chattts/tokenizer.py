...
class Tokenizer:
    ...
    def encode(
        ...
        for t in text:
            ...
            x = encode_plus(t, return_tensors="pt", add_special_tokens=False, padding=True)
        ...
        input_ids = torch.zeros(
            len(input_ids_lst), max_input_ids_len,
            device=device, dtype=input_ids_lst[0].dtype,
        )
        ...
        attention_mask = torch.zeros(
            len(attention_mask_lst), max_attention_mask_len,
            device=device, dtype=attention_mask_lst[0].dtype,
        )
    ...
    @torch.inference_mode()
    def decode(self, sequences: Union[List[int], List[List[int]]],
               skip_special_tokens: bool = False,
               clean_up_tokenization_spaces: bool = None, **kwargs):
