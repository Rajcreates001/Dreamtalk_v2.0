...
# Source: GPT_SoVITS/TTS_infer_pack/TextPreprocessor.py
...
class TextPreprocessor:
    ...
    def _get_lang_segmenter(self):
        if self.LangSegmenter is None:
            from dreamtalk.voice.core.tts.LangSegmenter import LangSegmenter as LS
            self.LangSegmenter = LS
        return self.LangSegmenter
    ...
    def get_phones_and_bert(self, text: str, language: str, version: str, final: bool = False):
        with self.bert_lock:
            ...
            textlist = []
            langlist = []
            LS = self._get_lang_segmenter()

            if language == "all_zh":
                for tmp in LS.getTexts(text, "zh"):
                    langlist.append(tmp["lang"])
                    textlist.append(tmp["text"])
            elif language == "all_yue":
                for tmp in LS.getTexts(text, "zh"):
                    langlist.append("yue" if tmp["lang"] == "zh" else tmp["lang"])
                    textlist.append(tmp["text"])
            elif language == "all_ja":
                for tmp in LS.getTexts(text, "ja"):
                    langlist.append(tmp["lang"])
                    textlist.append(tmp["text"])
            elif language == "all_ko":
                for tmp in LS.getTexts(text, "ko"):
                    langlist.append(tmp["lang"])
                    textlist.append(tmp["text"])
            elif language == "en":
                langlist.append("en")
                ...
                for tmp in LS.getTexts(text):
                    if language == "auto_yue" and tmp["lang"] == "zh":
                        tmp["lang"] = "yue"
                    langlist.append(tmp["lang"])
                    textlist.append(tmp["text"])
            else:
