...
fast_langdetect.infer._default_detector = fast_langdetect.infer.LangDetector(
    fast_langdetect.infer.LangDetectConfig(
        cache_dir=Path(__file__).parent.parent.parent.parent / "models" / "pretrained" / "fast_langdetect"
    )
)
...
def full_cjk(text):
    cjk_ranges = [
        ...
        (0x2A700, 0x2B73F), (0x2B740, 0x2B81F), (0x2B740, 0x2CEAF),
    ...
    for char in text:
        code_point = ord(char)
        if any(start <= code_point <= end for start, end in cjk_ranges) or re.match(pattern, char):
            cjk_text += char
    return cjk_text
