...
# Source: GPT_SoVITS/module/transforms.py
...
def piecewise_rational_quadratic_transform(
    ...
    outputs, logabsdet = spline_fn(
        inputs=inputs, unnormalized_widths=unnormalized_widths,
        unnormalized_heights=unnormalized_heights,
        unnormalized_derivatives=unnormalized_derivatives,
        inverse=inverse, min_bin_width=min_bin_width,
...
def rational_quadratic_spline(
    ...
    if min_bin_width * num_bins > 1.0:
        raise ValueError("Minimum bin width too large")
    if min_bin_height * num_bins > 1.0:
        raise ValueError("Minimum bin height too large")
...
def unconstrained_rational_quadratic_spline(
    ...
    if tails == "linear":
        ...
        raise ValueError(f"Unknown tails scheme: {tails}.")

    outputs[inside_interval_mask], logabsdet[inside_interval_mask] = rational_quadratic_spline(
        inputs[inside_interval_mask], unnormalized_widths[inside_interval_mask, :],
