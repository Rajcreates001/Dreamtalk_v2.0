...
import {
  ...
  Loader2, Mic, Pause, Play, RotateCcw, ScanFace, ShieldCheck, Sparkles,
  Square, Trash2, Upload, Volume2,
} from "lucide-react"
...
export function CreateTwinFlow() {
  ...
  return (
    <div className="relative min-h-dvh bg-background text-foreground overflow-x-hidden">
      ...
      <header className="sticky top-0 z-20 backdrop-blur-xl bg-background/70 border-b border-border-subtle">
        <div className="mx-auto max-w-5xl px-5 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {USE_MOCK && (
              <span className="label-mono rounded-full border border-border px-2.5 py-1 text-[10px]">
                Mock mode
              </span>
            )}
      ...
      <main className="mx-auto max-w-5xl px-5 pb-28 pt-10">
        {c.step === "consent" ? (
          ...
          <>
            <Stepper step={c.step} />
            <div className="mt-10">
              {/* Enter-only animation keyed by step. No exit gate, so a
                  backgrounded tab (paused rAF) can never strand the view. */}
              <motion.div
                key={c.step}
      ...
      {/* Footer nav */}
      {c.step !== "consent" && c.step !== "processing" && (
        <div className="fixed bottom-0 inset-x-0 z-20 border-t border-border-subtle bg-background/80 backdrop-blur-xl">
          <div className="mx-auto max-w-5xl px-5 h-20 flex items-center justify-between">
            <button
              onClick={c.goBack}
              disabled={STEP_ORDER.indexOf(c.step) <= 1}
            ...
            {c.step === "voice" ? (
              <button
                onClick={c.startProcessing}
                disabled={!c.canProceed.voice}
      ...
      {steps.map((key, i) => {
        ...
        return (
          <div key={key} className="flex flex-1 items-center gap-2 sm:gap-4">
            <div className="flex items-center gap-3">
              <div
                className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl border text-xs font-semibold transition-all ${
                  isActive
    ...
    <div className="mx-auto max-w-xl">
      ...
      >
        ...
        <h1 className="mt-3 font-display text-3xl font-bold leading-tight">
          Build your visual and vocal identity.
        </h1>
        ...
        <button
          onClick={() => c.setConsent(!c.consent)}
        ...
        >
          ...
          <span className="text-sm text-foreground-muted">
            ...
            I confirm that I have the right and permission to create and use this
            digital representation.
          </span>
        </button>

        <button
          onClick={() => c.setStep("identity")}
          disabled={!c.canProceed.consent}
    ...
    <div className="grid gap-8 lg:grid-cols-2">
      <div>
        ...
        <p className="mt-2 text-foreground-muted">
          Upload a clear, front-facing photo. Good lighting and a neutral
          expression produce the best digital twin.
        </p>
        ...
        {!c.facePreviewUrl && !camOpen && (
          <div
            ...
            className={`mt-6 rounded-3xl border-2 border-dashed p-10 text-center transition-all ${
          ...
          >
            ...
            <div className="mt-5 flex items-center justify-center gap-3">
              ...
              <button onClick={() => setCamOpen(true)}
                className="inline-flex items-center gap-2 rounded-xl border border-border bg-card px-4 py-2 text-sm font-medium hover:border-primary/40 transition-colors">
        ...
        {a && (
          <div className={`mt-6 rounded-2xl border p-4 ${
            a.status === "ready" ? "border-secondary/40 bg-secondary/8" : "border-warning/40 bg-warning/8"
          }`}>
            <div className="flex items-center gap-2 text-sm font-medium">
              {a.status === "ready"
                ? <><Check className="h-4 w-4 text-secondary" /> {a.message}</>
                : <><Info className="h-4 w-4 text-warning" /> {a.message}</>}
            </div>
            {a.status === "ready" && a.resolution && (
              <dl className="mt-3 grid grid-cols-3 gap-3 text-xs">
                <Stat k="Confidence" v={`${Math.round(a.confidence * 100)}%`} />
                <Stat k="Resolution" v={`${a.resolution.width}×${a.resolution.height}`} />
                <Stat k="Lighting" v={a.lighting ?? "—"} />
              </dl>
            )}
          </div>
        )}
            ...
            <>
              ...
              <div className="absolute inset-0 grid place-items-center">
                <span className="inline-flex items-center gap-2 rounded-full bg-background/80 px-3 py-1.5 text-xs font-medium">
                  <Loader2 className="h-3.5 w-3.5 animate-spin text-primary" /> Scanning face…
                </span>
              </div>
            </>
          )}
          {c.faceStatus === "done" && (
            <span className="absolute left-3 top-3 inline-flex items-center gap-1.5 rounded-full bg-secondary/90 px-2.5 py-1 text-[11px] font-semibold text-white">
              <Check className="h-3 w-3" /> Face detected
            </span>
          )}
        </div>
        {c.facePreviewUrl && (
          <button onClick={c.clearFace}
            className="absolute right-3 top-3 grid h-9 w-9 place-items-center rounded-full bg-background/80 text-foreground-muted hover:text-foreground transition-colors">
...
function CameraCapture({ onCapture, onCancel }: { onCapture: (f: File) => void; onCancel: () => void }) {
  ...
  return (
    <div className="mt-6 rounded-3xl border border-border bg-surface/40 p-4">
      {err ? (
        <p className="p-6 text-center text-sm text-warning">{err}</p>
      ) : (
        <>
          ...
          <div className="mt-3 flex justify-center gap-3">
            ...
            <button onClick={snap} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground">
...
function VoiceStep({ c }: { c: C }) {
  ...
  const onStop = async () => {
    ...
    if (r) c.setVoice(r.blob, r.duration)
  }
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-2">
      <div>
        ...
        <p className="mt-2 text-foreground-muted">
          Record or upload 8–15 seconds of clear speech. This becomes your twin&apos;s
          vocal identity.
        </p>
          ...
          {(["record", "upload"] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)}
              className={`rounded-lg px-4 py-1.5 text-sm font-medium capitalize transition-all ${
                tab === t ? "bg-primary/12 text-primary" : "text-foreground-muted hover:text-foreground"
              }`}>
              {t}
            </button>
          ))}
        ...
        {tab === "record" ? (
            ...
            <div className="mt-5 flex items-center justify-between">
              ...
              {rec.state === "recording" ? (
                ...
                <button onClick={rec.start}
                  className="inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-lg shadow-primary/20 hover:bg-primary-hover transition-all">
          ...
          <div className="mt-6 rounded-3xl border-2 border-dashed border-border bg-surface/40 p-10 text-center">
            ...
            <button onClick={() => uploadRef.current?.click()}
              className="mt-4 rounded-xl border border-border bg-card px-4 py-2 text-sm font-medium hover:border-primary/40 transition-colors">
              Browse files
            </button>
        ...
        {c.voiceUrl && (
          <div className="mt-4 flex items-center gap-3 rounded-2xl border border-border bg-card/70 p-3">
            <button onClick={() => { const el = audioRef.current; if (!el) return; playing ? el.pause() : el.play(); }}
              className="grid h-10 w-10 place-items-center rounded-full bg-primary/12 text-primary">
              {playing ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
            </button>
            <div className="flex-1">
              <div className="text-sm font-medium">Your recording</div>
              <div className="text-xs text-foreground-muted">{fmtTime(c.voiceDuration)} · {c.voiceAnalysis?.quality ?? "…"}</div>
            </div>
            <button onClick={c.clearVoice} className="grid h-9 w-9 place-items-center rounded-full text-foreground-muted hover:text-foreground">
              <Trash2 className="h-4 w-4" />
            </button>
        ...
        {c.voiceStatus === "analyzing" && (
          <p className="mt-3 inline-flex items-center gap-2 text-sm text-foreground-muted">
            <Loader2 className="h-4 w-4 animate-spin text-primary" /> Analyzing voice…
          </p>
        )}
        {c.voiceAnalysis && c.voiceStatus !== "analyzing" && (
          <p className={`mt-3 text-sm ${c.voiceStatus === "done" ? "text-secondary" : "text-warning"}`}>
            {c.voiceAnalysis.message}
          </p>
        )}
      </div>
      ...
      <div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {c.languages.length === 0 && (
            <span className="text-sm text-foreground-muted">Loading languages…</span>
          )}
          {c.languages.map((l) => {
            const on = c.selectedLangs.includes(l.code)
            return (
              <button key={l.code} onClick={() => c.toggleLang(l.code)}
                className={`rounded-full border px-3.5 py-2 text-sm transition-all ${
                  on ? "border-primary bg-primary/12 text-primary" : "border-border text-foreground-muted hover:border-primary/40 hover:text-foreground"
                }`}>
                <span className="font-medium">{l.native}</span>
                <span className="ml-1.5 text-xs opacity-70">{l.name}</span>
              </button>
            )
          })}
        </div>
...
function Waveform({ levels, active }: { levels?: number[]; active?: boolean }) {
  ...
  return (
    <div className="flex h-24 items-center justify-center gap-[3px]">
      {bars.map((v, i) => (
        <div key={i}
          className={`w-[3px] rounded-full transition-[height] duration-75 ${active ? "bg-primary" : "bg-border"}`}
...
// ═══════════════════════════════════════════════════════════════════
const PIPELINE = [
  { stage: "face_analysis", label: "Face analysis" },
  { stage: "reconstruction", label: "3D reconstruction" },
  { stage: "voice_processing", label: "Voice processing" },
  { stage: "speaker_profile", label: "Speaker profile" },
  { stage: "avatar_preparation", label: "Avatar preparation" },
] as const

function ProcessingStep({ c }: { c: C }) {
  const [showTech, setShowTech] = useState(false)
  const job = c.job
  const stageIdx = PIPELINE.findIndex((p) => p.stage === job?.stage)
  ...
  return (
    <div className="mx-auto max-w-2xl text-center">
      ...
      <p className="mt-2 text-foreground-muted">This usually takes a moment. Keep this tab open.</p>
      ...
      <div className="mt-8 space-y-2 text-left">
        {PIPELINE.map((p, i) => {
          const done = job?.status === "completed" || i < stageIdx
          const activeNow = i === stageIdx && job?.status !== "completed"
          return (
            <div key={p.stage} className="flex items-center gap-3 rounded-xl border border-border-subtle bg-card/50 px-4 py-3">
              <span className={`grid h-7 w-7 place-items-center rounded-lg ${
                done ? "bg-secondary/15 text-secondary" : activeNow ? "bg-primary/12 text-primary" : "bg-surface text-foreground-muted"
              }`}>
                {done ? <Check className="h-4 w-4" /> : activeNow ? <Loader2 className="h-4 w-4 animate-spin" /> : <span className="text-xs">{i + 1}</span>}
              </span>
              <span className={`text-sm ${done || activeNow ? "text-foreground" : "text-foreground-muted"}`}>{p.label}</span>
            </div>
          )
        })}
      </div>
      ...
      <div className="mt-6 text-left">
        ...
        <AnimatePresence>
          {showTech && job?.meta && (
            ...
            >
              <Stat k="Device" v={job.meta.device ?? "—"} />
              <Stat k="GPU" v={job.meta.gpu ?? "—"} />
              <Stat k="Model" v={job.meta.model ?? "—"} />
              <Stat k="Resolution" v={job.meta.resolution ?? "—"} />
              <Stat k="Inference" v={job.meta.inferenceMs ? `${job.meta.inferenceMs} ms` : "…"} />
              <Stat k="Status" v={job.status} />
            </motion.dl>
          )}
        </AnimatePresence>
      </div>
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <div className="text-center">
        ...
        <p className="mt-2 text-foreground-muted">Preview {c.avatar?.name}, then step into the studio to make them speak.</p>
      </div>
      ...
      <div className="mt-8 grid gap-6 sm:grid-cols-5">
        <div className="sm:col-span-2 aspect-[4/5] overflow-hidden rounded-3xl border border-border bg-surface/50">
          {c.facePreviewUrl ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={c.facePreviewUrl} alt={c.avatar?.name} className="h-full w-full object-cover" />
          ) : (
        ...
        <div className="sm:col-span-3 rounded-3xl border border-border bg-card/70 p-6">
          ...
          <h3 className="mt-1 font-display text-2xl font-bold">{c.avatar?.name}</h3>
          <dl className="mt-4 space-y-3 text-sm">
            <Row k="Visual identity" v={c.avatar?.visualIdentity ? "Ready" : "—"} good={c.avatar?.visualIdentity} />
            <Row k="Voice identity" v={c.avatar?.voiceIdentity ? "Ready" : "—"} good={c.avatar?.voiceIdentity} />
            <Row k="Status" v={c.avatar?.status ?? "—"} good={c.avatar?.status === "ready"} />
          </dl>
          ...
          <div className="mt-6 flex gap-3">
            <Link href="/studio" className="inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground hover:bg-primary-hover transition-all">
              Enter Astra Studio <ArrowRight className="h-4 w-4" />
            </Link>
            <Link href="/create-twin" className="inline-flex items-center gap-2 rounded-xl border border-border px-5 py-2.5 text-sm font-medium hover:border-primary/40 transition-colors">
...
function Stat({ k, v }: { k: string; v: string }) {
  return (
    <div className="rounded-lg bg-surface/60 px-3 py-2">
      ...
      <dd className="mt-0.5 font-medium capitalize text-foreground">{v}</dd>
    </div>
  )
}
