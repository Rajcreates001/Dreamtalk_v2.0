  ...
  return (
    <group position={position}>
      ...
      <mesh position={[0, 0, 0.04]}>
        ...
        <meshBasicMaterial color="var(--primary)" />
      </mesh>
      {/* Pupil */}
    ...
    <group ref={groupRef} position={[0, -0.3, 0]}>
      ...
      <group>
        ...
        <mesh ref={bodyRef} position={[0, 0.2, 0]}>
          ...
          <meshBasicMaterial color="var(--primary)" transparent opacity={0.12} />
        </mesh>

        {/* Chest glow */}
        <mesh position={[0, 0.35, 0.15]}>
          <circleGeometry args={[0.08, 16]} />
          <meshBasicMaterial color="var(--secondary)" transparent opacity={0.15} depthWrite={false} blending={THREE.AdditiveBlending} />
        </mesh>

        {/* Shoulders */}
        <mesh position={[-0.22, 0.4, 0]} rotation={[0, 0, 0.3]}>
          <capsuleGeometry args={[0.04, 0.15, 6, 8]} />
          <meshBasicMaterial color="var(--primary)" transparent opacity={0.08} />
        </mesh>
        <mesh position={[0.22, 0.4, 0]} rotation={[0, 0, -0.3]}>
          <capsuleGeometry args={[0.04, 0.15, 6, 8]} />
          <meshBasicMaterial color="var(--primary)" transparent opacity={0.08} />
        ...
        <mesh position={[0, 0.55, 0]}>
          ...
          <meshBasicMaterial color="var(--primary)" transparent opacity={0.1} />
        </mesh>
      </group>
      ...
      <group ref={headRef} position={[0, 0.65, 0]}>
        ...
        <mesh>
          ...
          <meshBasicMaterial color="var(--primary)" transparent opacity={0.15} />
        </mesh>

        {/* Wireframe overlay */}
        <mesh>
          <sphereGeometry args={[0.19, 16, 16]} />
          <meshBasicMaterial color="var(--primary)" transparent opacity={0.06} wireframe />
        </mesh>

        {/* Glow ring behind head */}
        <mesh ref={glowRef} position={[0, 0, -0.15]}>
          <circleGeometry args={[0.3, 24]} />
          <meshBasicMaterial color="var(--primary)" transparent opacity={0.08} depthWrite={false} blending={THREE.AdditiveBlending} />
        </mesh>
        ...
        <mesh position={[-0.07, 0.09, 0.18]}>
          ...
          <meshBasicMaterial color="var(--primary)" transparent opacity={0.2} />
        </mesh>
        <mesh position={[0.07, 0.09, 0.18]}>
          <boxGeometry args={[0.04, 0.005, 0.005]} />
          <meshBasicMaterial color="var(--primary)" transparent opacity={0.2} />
        ...
        <mesh position={[0, 0, 0]}>
          ...
          <meshBasicMaterial color="var(--secondary)" transparent opacity={0.04} depthWrite={false} blending={THREE.AdditiveBlending} />
        </mesh>
      </group>

      {/* Point light emanating from the being */}
      <pointLight position={[0, 0.5, 0.5]} color="var(--primary)" intensity={0.15} distance={3} decay={2} />
    </group>
  )
}
