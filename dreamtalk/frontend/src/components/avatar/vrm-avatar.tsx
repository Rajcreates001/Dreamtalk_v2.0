...
function SphereHead({ emotion, isSpeaking, lipSyncValue }: {
  ...
  const EMOTION_COLORS: Record<string, string> = {
    happy: "var(--warning)", sad: "#4169E1", angry: "#FF4444", fear: "#9932CC",
  ...
  return (
    <group ref={headRef}>
      ...
      <group position={[-0.18, 0.12, 0.48]}>
        ...
        <mesh position={[0, 0, 0.04]}><sphereGeometry args={[0.038, 16, 16]} /><meshStandardMaterial color="var(--foreground-muted)" /></mesh>
        <mesh position={[0, 0, 0.06]}><sphereGeometry args={[0.02, 12, 12]} /><meshStandardMaterial color="var(--accent-foreground)" /></mesh>
      ...
      <mesh ref={leftBrowRef} position={[-0.18, 0.52, 0.44]} rotation={[0, 0, 0.15]}>
        <boxGeometry args={[0.14, 0.025, 0.02]} /><meshStandardMaterial color="var(--surface-elevated)" />
      </mesh>
      <mesh ref={rightBrowRef} position={[0.18, 0.52, 0.44]} rotation={[0, 0, -0.15]}>
        <boxGeometry args={[0.14, 0.025, 0.02]} /><meshStandardMaterial color="var(--surface-elevated)" />
      </mesh>
      {/* Nose */}
      <mesh position={[0, 0.02, 0.55]}><sphereGeometry args={[0.035, 12, 12]} /><meshStandardMaterial color="var(--border)" /></mesh>
      {/* Mouth */}
      <mesh ref={mouthRef} position={[0, -0.14, 0.48]}>
        <boxGeometry args={[0.12, 0.02, 0.015]} /><meshStandardMaterial color="#CC7777" />
      </mesh>
      {/* Ears */}
      <mesh position={[-0.53, 0.05, 0.05]}><sphereGeometry args={[0.06, 12, 12]} /><meshStandardMaterial color="var(--border)" /></mesh>
      <mesh position={[0.53, 0.05, 0.05]}><sphereGeometry args={[0.06, 12, 12]} /><meshStandardMaterial color="var(--border)" /></mesh>
      {/* Neck */}
