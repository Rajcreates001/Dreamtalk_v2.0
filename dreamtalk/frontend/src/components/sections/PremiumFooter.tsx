...
export function PremiumFooter() {
  ...
  return (
    <footer className="relative z-10 border-t border-white/[0.04] bg-[#070B14]">
      {/* Top divider glow */}
      <div className="h-px bg-gradient-to-r from-transparent via-[#7C5CFF]/30 to-transparent" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-6 gap-8 mb-12">
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full bg-[#0F172A]/60 border border-white/[0.06] rounded-lg pl-9 pr-3 py-2 text-xs text-[#F8FAFC] placeholder:text-[#64748B] focus:outline-none focus:ring-1 focus:ring-[#7C5CFF]/30 transition-all"
                />
              </div>
              <motion.button
                className="px-3 py-2 rounded-lg bg-gradient-to-r from-[#7C5CFF] to-[#00E5FF] text-white text-xs font-medium"
          ...
          {footerColumns.map((col) => (
            <div key={col.title}>
              <h4 className="text-sm font-semibold text-[#F8FAFC] mb-4">{col.title}</h4>
              <ul className="space-y-2.5">
                {col.items.map((item) => (
                  <li key={item}>
                    <button className="text-sm text-[#64748B] hover:text-[#94A3B8] transition-colors">{item}</button>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-8 border-t border-white/[0.04]">
          <p className="text-xs text-[#64748B] flex items-center gap-1.5">
            © 2026 DreamTalk. Built with <Heart className="h-3 w-3 text-[#FF5F73]" /> for Digital Humans
          </p>
          <div className="flex items-center gap-4">
            {["Privacy", "Terms", "Cookies", "Licenses"].map((item) => (
              <button key={item} className="text-xs text-[#64748B] hover:text-[#94A3B8] transition-colors">
                {item}
              </button>
            ))}
            <motion.button
              onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
              className="w-8 h-8 rounded-lg bg-[#0F172A]/80 border border-white/[0.06] flex items-center justify-center text-[#64748B] hover:text-[#F8FAFC] transition-all"
              whileHover={{ y: -2 }}
            >
