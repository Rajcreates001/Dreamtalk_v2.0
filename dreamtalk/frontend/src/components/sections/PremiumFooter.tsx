...
export function PremiumFooter() {
  ...
  return (
    <footer className="relative z-10 border-t border-white/[0.04] bg-[#201D1D]">
      {/* Top divider glow */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-6 gap-8 mb-12">
          ...
          <div className="lg:col-span-2">
            ...
            <p className="text-sm text-[#8A8178] max-w-xs leading-relaxed mb-6">
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full bg-[#2C2929]/60 border border-white/[0.06] rounded-lg pl-9 pr-3 py-2 text-xs text-[#F3F4F4] placeholder:text-[#8A8178] focus:outline-none focus:ring-1 focus:ring-[#CC3A63]/30 transition-all"
                />
              </div>
              <motion.button
          ...
          {footerColumns.map((col) => (
            <div key={col.title}>
              <h4 className="text-sm font-semibold text-[#F3F4F4] mb-4">{col.title}</h4>
              <ul className="space-y-2.5">
                {col.items.map((item) => (
                  <li key={item}>
                    <button className="text-sm text-[#8A8178] hover:text-[#B0A79C] transition-colors">{item}</button>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-8 border-t border-white/[0.04]">
          ...
          <div className="flex items-center gap-4">
            {["Privacy", "Terms", "Cookies", "Licenses"].map((item) => (
              <button key={item} className="text-xs text-[#8A8178] hover:text-[#B0A79C] transition-colors">
                {item}
              </button>
            ))}
            <motion.button
              onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
              className="w-8 h-8 rounded-lg bg-[#2C2929]/80 border border-white/[0.06] flex items-center justify-center text-[#8A8178] hover:text-[#F3F4F4] transition-all"
              whileHover={{ y: -2 }}
            >
