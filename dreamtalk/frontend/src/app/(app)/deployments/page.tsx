  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-primary to-secondary text-white text-sm font-medium shadow-lg shadow-primary/20">
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold text-sm" style={{ background: `linear-gradient(135deg, ${dep.color}, color-mix(in srgb, ${dep.color} 50%, transparent))` }}>
                    {dep.name.split(" ").map(w => w[0]).join("").slice(0, 2)}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="text-sm font-semibold text-foreground">{dep.name}</h3>
                      <span className="px-2 py-0.5 rounded-md text-[10px] font-medium" style={{ background: `color-mix(in srgb, ${STATUS_CFG[dep.status].color} 8%, transparent)`, color: STATUS_CFG[dep.status].color }}>
                ...
                <div className="flex items-center gap-6">
                  <div className="text-right">
                    ...
                    <p className="text-sm font-semibold text-secondary">{dep.uptime}</p>
                  </div>
                  <div className="text-right">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Active Deployments", value: "2", icon: Globe, color: "var(--secondary)" },
          { label: "Total Requests", value: "102.7K", icon: Cpu, color: "var(--primary)" },
          { label: "Avg Uptime", value: "99.4%", icon: CheckCircle, color: "var(--secondary)" },
        ].map((stat) => (
