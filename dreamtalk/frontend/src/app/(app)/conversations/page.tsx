...
export default function ConversationsPage() {
  ...
  return (
    <div className="flex h-full -m-6 lg:-m-8 bg-background overflow-hidden">
      ...
      <div className="w-56 lg:w-64 shrink-0 border-r border-white/[0.06] flex flex-col bg-[#0F172A]/30 relative z-10">
        <div className="p-3 border-b border-white/[0.06]">
          <div className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-white/[0.04] border border-white/[0.06] text-[#64748B] text-sm">
            <Search className="h-3.5 w-3.5 shrink-0" />
            <input
              type="text"
              placeholder="Search conversations..."
              className="bg-transparent text-xs text-[#F8FAFC] placeholder:text-[#64748B] focus:outline-none w-full"
            />
          </div>
          <button
            type="button"
            className="w-full mt-2 flex items-center justify-center gap-1.5 py-2 rounded-lg bg-gradient-to-r from-[#7C5CFF]/20 to-[#00E5FF]/10 border border-[#7C5CFF]/20 text-xs text-[#7C5CFF] font-medium hover:from-[#7C5CFF]/30 transition-all"
          >
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {convLoading ? (
            ...
            >
              <p className="text-xs font-semibold text-[#CBD5E1] truncate">{conv.title}</p>
              <p className="text-[10px] text-[#64748B] mt-0.5 truncate">{conv.preview}</p>
              <p className="text-[9px] text-[#64748B] mt-1">{conv.time}</p>
            </button>
          )))}
        </div>
      </div>
      ...
      <motion.div
        ...
        className={cn(
          "relative flex flex-col border-r border-white/[0.06] bg-[#0F172A]/20 shrink-0 relative z-10",
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06] shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-xs font-semibold text-[#CBD5E1]">Dr. Aria</h2>
              <p className="text-[9px] text-[#64748B]">AI Companion</p>
            </div>
          </div>
          <button
            type="button"
          ...
          >
            <ArrowLeft className="h-3 w-3 text-[#64748B]" />
          </button>
        </div>
      ...
      {!avatarExpanded && (
        <button
          ...
          className="hidden lg:flex items-center justify-center w-6 h-12 self-center bg-[#0F172A]/40 border border-white/[0.06] rounded-r-lg text-[#64748B] hover:text-[#94A3B8] relative z-10 cursor-pointer"
        >
      ...
      <div className="flex-1 flex flex-col min-w-0 relative z-10">
        ...
        {/* Chat header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06] bg-[#0F172A]/30">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-xs font-semibold text-[#CBD5E1]">Dr. Aria</p>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-1.5 rounded-lg hover:bg-white/[0.06] text-[#64748B] hover:text-[#94A3B8] transition-all"
            >
              ...
              {isLoading && (
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 px-4 py-3 rounded-2xl bg-white/[0.04] border border-white/[0.06] rounded-tl-md">
                    <span className="h-2 w-2 rounded-full bg-[#64748B] animate-bounce" style={{ animationDelay: "0ms" }} />
                    <span className="h-2 w-2 rounded-full bg-[#64748B] animate-bounce" style={{ animationDelay: "150ms" }} />
                    <span className="h-2 w-2 rounded-full bg-[#64748B] animate-bounce" style={{ animationDelay: "300ms" }} />
                  </div>
                </motion.div>
              )}
            </div>
        ...
        {/* Input */}
        <div className="border-t border-white/[0.06] bg-[#0F172A]/20">
      ...
      <AnimatePresence>
        {showPanel && (
          <motion.div
            ...
            className="border-l border-white/[0.06] bg-[#0F172A]/30 overflow-hidden shrink-0 relative z-10"
          >
            <div className="w-[260px] p-4 space-y-4">
              ...
              <div className="p-3 rounded-xl bg-white/[0.03] border border-white/[0.06] space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-[#FF6B9D] font-medium">{THINKING_DATA.emotionScore}%</span>
                </div>
                <p className="text-xs text-[#CBD5E1]">{emotion || THINKING_DATA.emotion}</p>
                <div className="h-1.5 bg-white/[0.04] rounded-full overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${THINKING_DATA.emotionScore}%` }}
                    className="h-full rounded-full bg-gradient-to-r from-[#FF6B9D] to-[#7C5CFF]"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <span className="text-[10px] text-[#64748B] flex items-center gap-1.5">
                  <BookOpen className="h-3 w-3 text-[#00E5FF]" />
                  Knowledge Retrieved
                </span>
                <p className="text-[11px] text-[#CBD5E1]">{THINKING_DATA.knowledgeRetrieved}</p>
              </div>

              {/* Memory */}
              <div className="space-y-1.5">
                <span className="text-[10px] text-[#64748B] flex items-center gap-1.5">
                  <Brain className="h-3 w-3 text-[#7C5CFF]" />
                  Memory Used
                </span>
                <p className="text-[11px] text-[#CBD5E1]">{THINKING_DATA.memoryUsed}</p>
              </div>

              {/* Reasoning */}
              <div className="p-3 rounded-xl bg-white/[0.03] border border-white/[0.06]">
                <span className="text-[10px] text-[#64748B] flex items-center gap-1.5 mb-1.5">
                  <Cpu className="h-3 w-3 text-[#FBBF24]" />
                  Reasoning
                </span>
                <p className="text-[11px] text-[#94A3B8] leading-relaxed">{THINKING_DATA.reasoning}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 pt-2 border-t border-white/[0.06]">
                <div>
                  ...
                  <p className="text-xs font-semibold text-[#42FFC6]">{THINKING_DATA.confidence}%</p>
                </div>
                <div>
                  <p className="text-[9px] text-[#64748B]">Latency</p>
                  <p className="text-xs font-semibold text-[#F8FAFC]">{THINKING_DATA.latency}</p>
                </div>
              </div>
            </div>
          </motion.div>
