...
from dreamtalk.avatar.core.body.idol.lib.models.deformers.smplx.lbs import batch_rodrigues
import cv2
from PIL import Image
import numpy as np
...
def construct_camera(K, cam_list, device='cuda'):
    ...
    for i_cam in range(cam.shape[0]):
        poses.append( torch.concat([
            (intrics.reshape(-1)).to(torch.float32), #C ! # C ? T ç†è®ºä¸Šè¦ç»™C
            (cam_w2c[i_cam]).to(torch.float32).reshape(-1), # RT  #Rt|C ? RT ç†è®ºä¸Šè¦ç»™RT
        ], dim=0))
