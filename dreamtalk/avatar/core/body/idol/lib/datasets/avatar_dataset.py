...
# from dreamtalk.avatar.core.body.idol.lib.utils.train_util import print
...
class AvatarDataset(Dataset):
    ...
    def parse_scene(self, scene_id):
        ...
        if_use_smpl_param_ref = torch.Tensor([1]) # é»˜è®¤ä½¿ç”¨ref smpl, 
        if self.load_cond_data and len(cond_inds) > 0:
            ...
            if self.if_include_video_ref_img and input_is_video:
                # è®¾ç½®ä¸€ä¸ªéšæœºæ•°ï¼Œå¦‚æžœå°äºŽæŸä¸ªæ¦‚çŽ‡ï¼Œé‚£ä¹ˆæ›¿æ¢ç¬¬ä¸€å¼ å›¾ä¸ºå¦ä¸€ä¸ªå›¾ç‰‡
                if np.random.rand() < self.prob_include_video_ref_img:
                    ...
                    try:
                        ...
                        # replacement_img = load_image(ref_image_path)  # å‡è®¾æœ‰ä¸€ä¸ªå‡½æ•°å¯ä»¥åŠ è½½å›¾ç‰‡
                        # ä½¿ç”¨cv2.IMREAD_UNCHANGEDæ ‡å¿—è¯»å–å›¾ç‰‡ï¼Œä»¥ä¿ç•™alphaé€šé“
                        ...
                        if load_ref_smplx:
                            ...
                            # å‡è®¾ smpl_param_data æ˜¯å·²ç»åŠ è½½å¥½çš„æ•°æ® 
            ...
            if self.crop:
                ...
                if random.random() < 0.5:
                    ...
                    # ç¡®ä¿è£å‰ªåŒºåŸŸåœ¨å›¾åƒèŒƒå›´å†…
                    head_y = max(0, min(h, head_y))
                    body_y = max(0, min(h, body_y))

                    # è£å‰ªåŒºåŸŸçš„é«˜åº¦å’Œå®½åº¦
...
def gather_imgs(img_ids, poses, image_paths_or_video, smpl_params, load_imgs=True, load_norm=False, center=None, radius=None, input_is_video=False):
    ...
    for img_id in img_ids:
        ...
        c2w = pose.to(torch.float)#torch.FloatTensor(pose) # è™½ç„¶æ˜¯c2wä½†å…¶å®žå­˜çš„å€¼åº”è¯¥æ˜¯w2c (R|T)
        ...
        if input_is_video:
            ...
            if load_imgs:
                ...
                print("img.shape", img.shape)
                # å°†é€æ˜Žåƒç´ çš„RGBå€¼è®¾ç½®ä¸ºç™½è‰²ï¼ˆ255, 255, 255ï¼‰
...
#     front_direction = np.array([0, 0, -1])  # äººä½“æ­£é¢æ–¹å‘
#     min_angle = float('inf')
#     front_camera_idx = -1
...
def find_front_camera_by_rotation(poses, global_orient_human):
    # front_direction = global_orient_human  # äººä½“æ­£é¢æ–¹å‘
    rotation_matrix = axis_angle_to_rotation_matrix(global_orient_human)
    front_direction = rotation_matrix @ torch.Tensor([0, 0, -1])  # äººä½“çš„æœå‘
    min_angle = float('inf')
    front_camera_idx = -1

    for idx, pose in enumerate(poses):
        rotation_matrix = pose[0][:3, :3]
        camera_direction = rotation_matrix @ torch.Tensor([0, 0, 1])  # ç›¸æœºçš„æœå‘
...
def from_video_to_get_ref_smplx(video_path):
    ...
    video_name = video_dir.split("/")[-1] # è§†é¢‘æ–‡ä»¶å¤¹åç§°
    
    # æ›¿æ¢è§†é¢‘ç›®å½•ä¸º smplify ç›®å½•
    ...
    # # èŽ·å–è§†é¢‘æ–‡ä»¶åï¼ˆä¸åŒ…æ‹¬æ‰©å±•åï¼‰
...
def random_scale_and_crop(image: torch.Tensor, scale_range=(0.8, 1.2)) -> torch.Tensor:
    ...
    # ä½¿ç”¨ torchvision.transforms.functional.resize è¿›è¡Œç¼©æ”¾
    scaled_image = F.resize(image.permute(2, 0, 1), [new_h, new_w]).permute(1, 2, 0)

    # å¦‚æžœç¼©æ”¾åŽçš„å›¾åƒæ¯”åŽŸå›¾å¤§ï¼Œè¿›è¡Œå±…ä¸­è£å‰ª
    if new_h > h or new_w > w:
        top = (new_h - h) // 2
        left = (new_w - w) // 2
        scaled_image = scaled_image[top:top + h, left:left + w]
    else:
        # å¦‚æžœç¼©æ”¾åŽçš„å›¾åƒæ¯”åŽŸå›¾å°ï¼Œè¿›è¡Œå±…ä¸­å¡«å……
        padded_image = torch.ones((h, w, 3), dtype=image.dtype)
        top = h-new_h #(h - new_h) // 2 # Hä¸åº”è¯¥å±…ä¸­
        left = (w - new_w) // 2
