...
from dreamtalk.avatar.core.body.idol.lib.mmutils import xavier_init, constant_init
import numpy as np
import time
import cv2
import math
...
from dreamtalk.avatar.core.body.idol.lib.ops import TruncExp
import torchvision
...
class UVNDecoder_gender(nn.Module):
    ...
    def __init__(self,
        ...
        if reshape_type=='cnn':
            self.upsample_conv = torch.nn.ConvTranspose2d(512, 32, kernel_size=4, stride=4)
                                                 
        elif reshape_type == 'VitHead': # changes the up block's layernorm into the feature channel norm instead of the full image norm
            from dreamtalk.avatar.core.body.idol.lib.models.decoders.vit_head import VitHead
