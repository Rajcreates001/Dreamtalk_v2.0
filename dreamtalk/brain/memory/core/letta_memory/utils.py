...
from dreamtalk.brain.memory.core.letta_memory import __path__ as __letta_path__
...
from dreamtalk.brain.memory.core.letta_memory.helpers.json_helpers import json_dumps, json_loads
from dreamtalk.brain.memory.core.letta_memory.log import get_logger
from dreamtalk.brain.memory.core.letta_memory.otel.tracing import log_attributes, trace_method
from dreamtalk.brain.memory.core.letta_memory.schemas.openai.chat_completion_response import ChatCompletionResponse
from dreamtalk.brain.memory.core.letta_memory.server.rest_api.dependencies import HeaderParams
...
class OpenAIBackcompatUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        if module == "openai.openai_object":
            from dreamtalk.brain.memory.core.letta_memory.openai_backcompat.openai_object import OpenAIObject
...
def list_human_files():
    ...
    defaults_dir = os.path.join(__letta_path__[0], "humans", "examples")
...
def list_persona_files():
    ...
    defaults_dir = os.path.join(__letta_path__[0], "personas", "examples")
...
def get_friendly_error_msg(function_name: str, exception_name: str, exception_message: str):
    from dreamtalk.brain.memory.core.letta_memory.constants import MAX_ERROR_MESSAGE_CHAR_LIMIT
...
def log_telemetry(logger: Logger, event: str, **kwargs):
    ...
    """
    from dreamtalk.brain.memory.core.letta_memory.settings import log_settings
...
async def decrypt_agent_secrets(agents: list) -> list:
    ...
    async def decrypt_env_var(env_var):
        from dreamtalk.brain.memory.core.letta_memory.orm.agent import ENCRYPTED_PLACEHOLDER
