...
from dreamtalk.brain.memory.core.letta_memory.__init__ import __version__ as letta_version
from dreamtalk.brain.memory.core.letta_memory.agents.exceptions import IncompatibleAgentType
from dreamtalk.brain.memory.core.letta_memory.constants import ADMIN_PREFIX, API_PREFIX
...
from dreamtalk.brain.memory.core.letta_memory.helpers.json_helpers import sanitize_unicode_surrogates
from dreamtalk.brain.memory.core.letta_memory.helpers.pinecone_utils import get_pinecone_indices, should_use_pinecone, upsert_pinecone_indices
from dreamtalk.brain.memory.core.letta_memory.jobs.scheduler import start_scheduler_with_leader_election
from dreamtalk.brain.memory.core.letta_memory.log import get_logger
from dreamtalk.brain.memory.core.letta_memory.orm.errors import (
...
from dreamtalk.brain.memory.core.letta_memory.otel.tracing import get_trace_id
from dreamtalk.brain.memory.core.letta_memory.schemas.letta_message import create_letta_error_message_schema, create_letta_message_union_schema
from dreamtalk.brain.memory.core.letta_memory.schemas.letta_message_content import (
...
from dreamtalk.brain.memory.core.letta_memory.server.constants import REST_DEFAULT_PORT
...
from dreamtalk.brain.memory.core.letta_memory.server.global_exception_handler import setup_global_exception_handlers

# NOTE(charles): these are extra routes that are not part of v1 but we still need to mount to pass tests
from dreamtalk.brain.memory.core.letta_memory.server.rest_api.auth.index import setup_auth_router  # TODO: probably remove right?
from dreamtalk.brain.memory.core.letta_memory.server.rest_api.interface import StreamingServerInterface
from dreamtalk.brain.memory.core.letta_memory.server.rest_api.middleware import CheckPasswordMiddleware, LoggingMiddleware, RequestIdMiddleware
from dreamtalk.brain.memory.core.letta_memory.server.rest_api.routers.v1 import ROUTERS as v1_routes
from dreamtalk.brain.memory.core.letta_memory.server.rest_api.routers.v1.organizations import router as organizations_router
from dreamtalk.brain.memory.core.letta_memory.server.rest_api.routers.v1.users import router as users_router  # TODO: decide on admin
from dreamtalk.brain.memory.core.letta_memory.server.rest_api.static_files import mount_static_files
from dreamtalk.brain.memory.core.letta_memory.server.rest_api.utils import SENTRY_ENABLED
from dreamtalk.brain.memory.core.letta_memory.server.server import SyncServer
from dreamtalk.brain.memory.core.letta_memory.settings import settings, telemetry_settings
from dreamtalk.brain.memory.core.letta_memory.validators import PATH_VALIDATORS, PRIMITIVE_ID_PATTERNS
...
async def lifespan(app_: FastAPI):
    ...
    """
    worker_id = os.getpid()
    from dreamtalk.brain.memory.core.letta_memory.monitoring.readiness_state import initialize_readiness_state, set_readiness_state
    ...
    try:
        ...
        from dreamtalk.brain.memory.core.letta_memory.monitoring.event_loop_watchdog import start_watchdog
    ...
    try:
        ...
        from dreamtalk.brain.memory.core.letta_memory.otel.db_pool_monitoring import setup_pool_monitoring
        from dreamtalk.brain.memory.core.letta_memory.server.db import db_registry, engine as db_engine
    ...
    try:
        from dreamtalk.brain.memory.core.letta_memory.server.rest_api.routers.v1.git_http import set_server_instance
    ...
    try:
        from dreamtalk.brain.memory.core.letta_memory.monitoring.event_loop_watchdog import stop_watchdog
    ...
    try:
        from dreamtalk.brain.memory.core.letta_memory.jobs.scheduler import shutdown_scheduler_and_release_lock
    ...
    if not settings.disable_tracing and settings.sqlalchemy_tracing:
        try:
            from dreamtalk.brain.memory.core.letta_memory.otel.sqlalchemy_instrumentation_integration import teardown_letta_db_instrumentation
...
def create_application() -> "FastAPI":
    ...
    if otlp_endpoint and not settings.disable_tracing:
        ...
        from dreamtalk.brain.memory.core.letta_memory.otel.metrics import setup_metrics
        from dreamtalk.brain.memory.core.letta_memory.otel.tracing import setup_tracing
        ...
        if settings.sqlalchemy_tracing:
            from dreamtalk.brain.memory.core.letta_memory.otel.sqlalchemy_instrumentation_integration import setup_letta_db_instrumentation
...
def start_server(
    ...
    if debug:
        from dreamtalk.brain.memory.core.letta_memory.server.server import logger as server_logger
