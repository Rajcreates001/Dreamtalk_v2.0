...
def _create_backend(backend: ProviderTraceBackend | str) -> ProviderTraceBackendClient:
    ...
    match backend_str:
        case "clickhouse":
            from dreamtalk.brain.memory.core.letta_memory.services.provider_trace_backends.clickhouse import ClickhouseProviderTraceBackend

            return ClickhouseProviderTraceBackend()

        case "socket":
            from dreamtalk.brain.memory.core.letta_memory.services.provider_trace_backends.socket import SocketProviderTraceBackend

            return SocketProviderTraceBackend(socket_path=telemetry_settings.socket_path)

        case "postgres" | _:
            from dreamtalk.brain.memory.core.letta_memory.services.provider_trace_backends.postgres import PostgresProviderTraceBackend
