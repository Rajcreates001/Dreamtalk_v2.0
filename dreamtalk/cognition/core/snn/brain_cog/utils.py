...
def adjust_surrogate_coeff(epoch, tot_epochs):
    ...
    t = torch.tensor([math.pow(10, Kmin + (Kmax - Kmin) / tot_epochs * epoch)]).float().to(_dev)
    k = torch.tensor([1]).float().to(_dev)
    if k < 1:
        k = 1 / t
    return t, k
...
def calc_aurc(confidences, labels):
    ...
    _dev = labels.device
    risk_cov = torch.divide(torch.cumsum(labels != predictions,dim=0).float(), torch.arange(1, n+1, device=_dev))
    nrisk = torch.sum(labels != predictions)
    aurc = torch.mean(risk_cov)
    opt_aurc = (1./n) * torch.sum(torch.divide(torch.arange(1, nrisk + 1, device=_dev).float(), n - nrisk + torch.arange(1, nrisk + 1, device=_dev)))
    eaurc = aurc - opt_aurc
