...
from ..core.session_extractor import extract_memorable_items
...
from ..infrastructure.scanner import read_head_tail
...
async def _import_file(
    ...
    from ..core.session_extractor import extract_session_summary
...
async def _process_imports(
    ...
    if run_pipeline and total_imported > 0:
        try:
            from .wiki_pipeline import handler as _pipeline
...
async def handler(args: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if result.get("backfilled", 0) > 0:
        try:
            from .consolidation.cascade import (
