...
def _load_user_rules():
    ...
    try:
        ...
        from .wiki_schema_loader import load_registry
...
def _apply_user_rules(content: str, tags: list[str] | None):
    ...
    if not rules:
        return None
    from .wiki_rule_engine import apply_rules
...
def _detect_modern_kind(
    ...
    """
    from .wiki_axis_registry import AXIS_KIND, get_registry, match_axis
...
def _detect_provenance(tags: list[str] | None) -> str:
    ...
    """
    from .wiki_axis_registry import (
        AXIS_PROVENANCE,
        get_registry,
        match_axis,
    )
    ...
    **Open-world dispatch.** Every axis consults
    ...
    Admission gates (unchanged from the legacy single-kind classifier):
      ...
      5. Positive scoring — ≥ 4 of 8 signals, unless an explicit knowledge
         tag bypasses.
    """
    from ..shared.wiki_classification import Classification, Generator
    ...
    from .wiki_axis_registry import AXIS_PROVENANCE, get_registry
