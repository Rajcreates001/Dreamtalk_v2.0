...
from .utils import load_state_dict, init_weights_on_device, hash_state_dict_keys, split_state_dict_with_prefix, smart_load_weights
...
class ModelDetectorTemplate:
    ...
    def load(self, file_path="", state_dict={}, device=None, torch_dtype=torch.float16, **kwargs):
        if device is None:
            import os
            device = os.environ.get("DREAMTALK_DEVICE") or ("cuda" if torch.cuda.is_available() else "cpu")
        return [], []
...
class ModelDetectorFromSingleFile:
    ...
    def load(self, file_path="", state_dict={}, device=None, torch_dtype=torch.float16, infer=False, **kwargs):
        if device is None:
            import os
            device = os.environ.get("DREAMTALK_DEVICE") or ("cuda" if torch.cuda.is_available() else "cpu")
        if len(state_dict) == 0:
